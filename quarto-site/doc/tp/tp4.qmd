...
### Quick Browser Check
The simplest way to interact with an API is via your browser. However, browsers are limited to **GET** requests.

- [ ] Open your browser and go to: `https://api.zippopotam.us/us/90210`
- [ ] Observe the response. It is in **JSON** format.
- [ ] **Question:** Can you use the address bar to send a `POST` request to create a new user? (Answer: No, browsers are primarily GET clients).

### Professional API Clients
To perform more complex actions (`POST`, `PUT`, `DELETE`), you need a specialized tool.
**Task:** Use [Hoppscotch](https://hoppscotch.io/), [Insomnia](https://insomnia.rest/), or [Bruno](https://www.usebruno.com/) to test the following:

- [ ] **GET** `https://api.carbonintensity.org.uk/intensity`
- [ ] **GET** `https://api.carbonintensity.org.uk/intensity/date/2023-05-01` (Replace with a real date).
- [ ] **Analyze the Status Code:** If you request a date in the future, what code do you receive? (e.g., 400, 404, or 500?)

---

## Using Swagger (The Documentation)
...
1.  Launch your backend.
2.  Go to: `http://localhost:8000/docs` (or your specific port).

**Task:** Use the Swagger interface to:
- [ ] Find a player by ID.
- [ ] Create a new player.
- [ ] Observe how Swagger automatically generates the **JSON Body** for you.


## The Client Challenge (Data Mapping)

Now, we will move to a more advanced task. You are going to act as a **Client**. You need to fetch game data from an external source and integrate it into your engine.
...
### The Mission
Your task is to create a utility in `backend/src/utils/external_game_client.py`. This client must fetch data from our external mock API and transform it into your `business_object.Game` objects.

**The Data Gap:**
| External API Field (JSON) | Your `Game` Object Field |
| :--- | :--- |
| `match_uuid` | `id_game` |
| `players_list` (array) | `player1` and `player2` (Objects) |
| `winner_name` | `winner` (Object) |
| `location_name` | `location` (String) |
| `duration_seconds` | `duration` (Integer) |

**Tasks:**
- [ ] Implement `get_remote_games() -> list[Game]`.
- [ ] **The Mapper:** Inside your client, write the logic to convert the "ugly" external JSON into your "clean" `Game` objects.
- [ ] **Error Handling:** If the external API returns a `500` or `404`, your client should catch it and return an empty list instead of crashing the whole app.
