...
async def create_game(
    ...
    token: str = Header(None) # 1. Capture the header
):
...
- [ ] **Create Models**: Define `GameCreate` and `GameResponse` in `game_model.py`.
- [ ] **Implement Controller**: Create `game_controller.py` using the `GameService`.
- [ ] **Apply Validation**: Use your Pydantic models in the function arguments.
