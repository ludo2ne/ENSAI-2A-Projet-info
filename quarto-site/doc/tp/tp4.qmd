...
Last week, you primarily queried web services to retrieve data in JSON format, which you then converted into Python business objects.
...
::: {.callout-tip title="If you don't have branch tp4-base"}
...
- [ ] *find_attack_by_id(id:int) -> AbsractAttack*: returns the attack with the given ID or `None` if the attack is not found
- [ ] *find_all_attacks() -> List[AbsractAttack]*: returns a list of all attacks
  - [Bonus]{.underline}: add `limit` and `offset` parameters to this method
- [ ] *update_attack(attack : AbstractAttack) -> bool*: updates the data of the attack passed as a parameter and returns whether the modification was successful
...
- Remember to run tests to see if your code works
:::
...
- [ ] *find_all_pokemon()->List[AbstractPokemon]*: returns all Pokemon in the database
- [ ] *find_pokemon_by_name(name:str)->AbstractPokemon*: returns a Pokemon with the given name
...
- `GET localhost:80/attack?limit=100` will return a list of all attacks
- `GET localhost:80/pokemon?limit=100`. It will return a list of 100 *Pokemons* by default, but can be adjusted with the `limit` query parameter
- `GET localhost:80/pokemon/{name}`. It will return a JSON representing a *Pokemon*
