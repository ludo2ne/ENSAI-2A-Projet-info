...
Imagine you are a customer trying to interact with a shop assistant; this simple interaction is a good metaphor for the client-server relationship and the world of APIs.
...
In computer science, the principle is exactly the same: 

- a client sends requests to a server
- the server processes these requests 
- and sends back a response — either successfully or with an error explaining why the request could not be fulfilled.
...
| **401** | Client Error | **Unauthorized**: Authentication required and has failed or not been provided. |
...
: {tbl-colwidths="[10,15,75]" .striped .hover .secondary}
...
> Enough playing with lightsabers and Pikachu – let's get back to a serious topic.
...
Up until now, you have used `GET` requests to *ask* for information (the same goes for `DELETE`). 

```{.txt filename="Examples"}
GET https://swapi.info/api/people
DELETE https://pokeapi.co/api/v2/pokemon/114
```

But to *create* (`POST`) or *modify* (`PUT`) data, you must provide further information, not included in the URL. This information will be stored in the **Request Body**.

```{.json filename="Example of JSON Body"}
{
...
::: {.callout-tip}
The different parts of an HTTP request:

- Method → what we want to do
- URL → which resource we want to access
- Headers → provide additional information about the request, such as authentication or the content type
- Body → contains the data sent to the server, when needed

```{.txt}
POST https://chess-boxing.com/players                   <- Method + URL

Content-Type: application/json                          <- Headers
Authorization: Bearer AbC1459mlkP78sq25z8
...
- [ ] Get json (*r.json()*), convert into a list of Game, and finally return it
...
::: {.callout-tip}
After each step, feel free to test the endpoint via the swagger.
:::
...
OK, cool. We already have models for Game. But why do we need multiple models?

Why not simply define one model for Game and use it everywhere — for creating, updating, and reading a game?
