...
> Since all of you have reviewed your relational database course this summer :tropical_drink: :book: :sunglasses:
...
SELECT g.*,
       p.id_player = g.id_winner AS win
  FROM game g
  JOIN player p ON (p.username = 'batricia' AND (g.id_player1 = p.id_player OR g.id_player2 = p.id_player))
 ORDER BY g.timestamp DESC;

SELECT pokemon_fan,
       COUNT(*)
  FROM player
 GROUP BY pokemon_fan;
```
:::
...
- [ ] Play a game
- [ ] Make sure everything looks OK in the logs
- [ ] Check if a row was created in the database

### New Service methods

Using the new DAO methods we've implemented, let's create a few service methods. In the next lab, we'll use them in the controller to create new endpoints.

In `GameService`, implement methods:

- [ ] *find_by_id(id_game) -> Game*

::: {.callout-tip}
This methods is very simple; it calls the appropriate DAO and return its response.

This may seem like "useless code" (as we are simply calling another function), it is a fundamental practice in software architecture for the following reasons:

- Decoupling: It keeps the Controller from talking directly to the Database, maintaining a clean layered architecture
- Centralized Control: It provides a single place to later add caching, security checks, or logging without touching the Controller
:::

- [ ] *find_all_by_player(id_player, game_mode) -> list[Game]*
  - returning all games by *id_player* and *game_mode*
  - if *game_mode* is None, return all player games
- [ ] Write the unit tests for this last method
  - Don't forget to mock DAO calls
...
> "I want to test the *delete()* method. I'll create a game, then delete it. Easy!" :sunglasses:
...
> "I'll just run my tests directly on my database where I have all my useful data." :bomb:
...
This code temporarily redirect database operations to a dedicated test schema, ensuring that the production data remains untouched. It initialize the database with a fresh set of test data, providing a predictable and isolated state for all subsequent tests.
