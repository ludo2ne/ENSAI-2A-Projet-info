...
- [ ] `GET localhost:80/character`: will return a JSON containing a list of characters
- [ ] `PUT localhost:80/character/{id}`, which will modify the name of the character at index `{id}` from a body
- [ ] `DELETE localhost:80/character/{id}`, which will delete the item at index `{id}`
