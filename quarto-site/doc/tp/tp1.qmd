---
title: Git reminder, Example of Application
description: Lab 1
...
- [ ] Optional: follow these [instructions](https://ludo2ne.github.io/ENSAI-tools/doc/datalab/vscode-python.html#ia-agent){target="_blank"} to **set up** AI agent.


::: {.callout-important}
LLMs are a limited resource. Please use them **moderately and responsibly**.

- **Models hosted on SSP Cloud**
  The available computing power is limited, so the resources allocated are also limited. Avoid unnecessary or intensive requests.
- **External providers** (e.g. Mistral, ChatGPT)
  Free access is subject to a limited number of tokens. Once this quota is reached, the service may no longer be available for free.

**In all cases:** use LLMs when they are genuinely useful, and avoid unnecessary requests.
:::
...
You will be working on a VSCode-Python service, which will be where your local repository will be located.

Let's start by launching the service.
...
- [ ] In the catalog, select **VSCode-python** and click [Launch]{.blue-button} **[once]{.underline}**
- [ ] **Initialization script:** Fill url with `https://raw.githubusercontent.com/ludo2ne/ENSAI-2A-projet-info-template/refs/heads/main/onyxia_setup.sh`
  - This script will install/uninstall VSCode extensions

::: {.callout-note}
[More about initialization script](https://ludo2ne.github.io/ENSAI-tools/doc/datalab/advanced.html){target="_blank"}
:::

- [ ] **Git:** Fill **Repository url** using https url of your remote repository
- [ ] **Git:** Select *Open the service in the clone folder*
- [ ] **Network:** expose ports 5000 and 8000
- [ ] **User preferences:** Enable AI Assistant
- [ ] [Launch]{.blue-button} and [Open]{.blue-button} the service
...
> Example of link: <https://datalab.sspcloud.fr/launcher/ide/vscode-python?name=vscode-python&version=2.6.2&s3=default&init.personalInit=«https%3A%2F%2Fraw.githubusercontent.com%2Fludo2ne%2FENSAI-2A-projet-info-template%2Frefs%2Fheads%2Fmain%2Fonyxia_setup.sh»&git.repository=«https%3A%2F%2Fgithub.com%2Fludo2ne%2FENSAI-2A-projet-info-template.git»&git.asCodeServerRoot=true&networking.user.enabled=true&networking.user.ports[1]=8000&autoLaunch=true>{target="_blank"}
...
- [ ] Open a new terminal
  ...
  - or use shortcut (CTRL + ù or CTRL + SHIFT + C)
...
Normally, the root folder of your file explorer is your repository (since you checked option *Open the service in the clone folder*).

- [ ] If not, position yourself in your repo
...
This opens a new tab (you can close the previous one) with your project set as the root of your file explorer.
...
| .vscode/       | Editor settings, extensions, and debugger configurations.             |
...
| File                | Description |
| ------------------- | ----------- |
| README.md           | To explain the architecture and development tools.                |
...
| logging_config.yml  | Configuration file defining how the application logs information. |
...
::: {.callout-important}
The root README provides high-level instructions for users to launch the application, while the README files located in the backend/ and frontend/ directories contain technical documentation intended for developers.
:::
...
- [ ] Feel free to modify for example the theme or the font size
...
- **Dependencies:** Core libraries required to run the app 
  - including dev libraries (needed only for development, not for running the application)
...
As part of your project, you will need to fill out this file.
...
- **Web services**: Interaction via API endpoints to retrieve data
...
We will discuss webservices in more detail in Lab 4.
...
- [ ] Open a new terminal
- [ ] Launch `curl http://localhost:5000/hello/Chewbacca`
  - add ` | jq .` at the end for pretty print
  - add option `-v` after *curl*: verbose mode

Then from the outside:

- [ ] Go back to [My service](https://datalab.sspcloud.fr/my-services){target="_blank"} page
- [ ] Click on [Open]{.blue-button} button of the service
- [ ] Click on *Port 5000* link

::: {.callout-note}
Using this link, even your neighbour can access it.
:::
...
If not, check the terminal in which you launched the application. Perhaps you forgot to reset the database.

- [ ] Search for players by their ID
  - Try IDs 6 and 99


::: {.callout-tip title="HTTP Status codes" collapse="true"}
More details in Lab4.
...
: {tbl-colwidths="[15,15,15,55]" .striped .hover .secondary}
:::
...
The diagram below shows how a request to fetch a player travels through the application layers, from the API router to the database.
...
- [ ] Open a new terminal
  - :bulb: F2 or Right click to rename terminals
- [ ] Follow frontend README instructions to run the menu application
...
- [ ] Find and change the password required length from 35 to 16 characters
  - :mag: :mag: :mag: Search through the code
...
- [ ] Creates a local clone of the repository
  - In the same VSCode service (:warning: go back to the work folder `cd $HOME/work/` and use your token)
  - Or in a new one
- [ ] Does a test to check that they can push code, for example by creating or modifiying:
  ...
  - *README.md*
  - *main.py*
- [ ] Add the teacher's repo as a secondary remote named *upstream*
  - `git remote add upstream https://github.com/ludo2ne/ENSAI-2A-projet-info-template.git`
  - If the teacher updates the template, it will be useful for retrieving the changes

::: {.callout-caution}
Afterwards, you'll make sure that your repo is "tidy", in the same way as described in the previous section.
:::
...
- [ ] Is the *.env* file intended to be uploaded to the remote repository?
- [ ] Take appropriate action


### PostgreSQL database

> It's not yet time to create your database; here are just a few basic principles.
...
| Pros | Cons |
| ---- | ---- |
| **Zero setup:** Everyone connects to the same URL. | **High Risk:** One accidental `DROP TABLE` or `DELETE` breaks the project for everyone.  |
| **Data Sync:** Everyone sees the same rows. | **Conflict:** You can't test "deleting a user" if your teammate is currently using that user. |
...
| Pros | Cons |
| ---- | ---- |
| **Safety:** You can break your local table without consequences.  | **Sync Effort:** You must remember to `git commit` the `.sql` file every time you change a column.  |
| **Independence:** You can test different data states simultaneously. | **Drift:** If someone forgets to commit, the team's environments will diverge. | 
