...
:construction: Each member of the team works on its own branch
...
classDiagram
  class GameService {
    ...
    + play(player_id, opponent_id, mode) Game
  }

  class GameModeFactory {
    + get_mode(mode) GameMode
  }
...
- [ ] Implement the constructor with all attributes as parameters except *id*
  ...
  - *mode*: The type of game played ("coinflip" or "dice")
...
Fine, we'll also need a way to generate either a *CoinFlipMode* object or a *DiceMode* object based on a *mode* (str) parameter. To do this, let's create a factory class:

- [ ] Create a `GameModeFactory` class 
- [ ] Add static method called `get_mode(mode: str, **kwargs) -> GameMode`
  - that takes the mode (str) as parameter
...
stateDiagram
    direction LR
    [*] --> GameModeFactory : mode (str)
    GameModeFactory --> CoinFlipMode : if "mode=coinflip"
    GameModeFactory --> DiceMode : if "mode=dice"
```
