...
stateDiagram
    ...
    GameModeFactory --> CoinFlipMode(tails) : if "game_mode=coinflip" <br> and "choice=tails"
...
::: {.callout-note}
The methods *update_elo()*, *compute_elo()* and *expected_score()* do not belong in the `GameService` class.
...
- [ ] Update method *update_player_ratings(game)*, replacing parameters by a `Game` object
- [ ] In `GameService`, call that method
  - Anyway, it's better to keep the DAO calls within the service (*PlayerDao().update()*)
