...
- [ ] Add *\_\_str\_\_()* method returning for example:
...
Ensuring code correctness isn't always straightforward. Occasionally, manual testing can be more efficient than writing full unit tests for simple logic. As a workaround, you can use a sandbox.py file in the src folder for rapid prototyping.
...
and run it using : `uv run --project backend python backend/src/sandbox.py`
:::
...
Next, let's create the two child classes that will inherit from `GameMode`
...
:::{.callout-tip title="Imports"}
An import allows you to use code from one file in another. For example, if your CoinFlipMode class needs to create a 
Game object, you must import the Game class.

You can use absolute imports from root folder `src` (recommended) or relative ones.

`from business_object.game import Game` means :

- From folder *business_object*
- File *game* (.py)
- Import class `Game`
:::
...
Feel free to try it manually :

- [ ] Launch the backend
- [ ] Go to the swagger page and use `/login` endpoint
  - Copy the token
- [ ] Play a Game against someone else
