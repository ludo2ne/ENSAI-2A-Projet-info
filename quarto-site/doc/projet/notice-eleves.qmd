---
title: "Notice élèves"
description: Projet informatique 2A 2025-2026
author:
  - Romaric Gaudel
  - Rémi Pépin
  - Ludovic Deneuville
...
Le travail demandé consiste à construire une application permettant de répondre à la problématique du sujet proposé. Ce travail se décompose en 2 grandes phases :

1. Étude préalable et conception générale
  - expliquer le besoin
  - décrire la solution envisagée
  - planifier les grandes phases de la réalisation
  - décrire les exigences fonctionnelles par la modélisation (diagrammes UML)
  - planifier la mise en place des fonctionnalités (dépendances, priorités...)
2. Réalisation
  - mise en place de la base de données
  - développement de l'application en Python


::: {.callout-note title="Travaux à rendre"}
À mi-projet, vous livrerez le dossier d'analyse correspondant au travail réalisé lors de la première phase.
...
Les équipes sont composées par le responsable de la matière en tenant compte des niveaux constatés en informatique lors de la première année et en veillant à la répartition des élèves admis directement en 2e année. Il est essentiel de veiller à la bonne intégration des élèves admis sur titre au sein des équipes.
...
Vous serez encadré par une tutrice ou un tuteur qui suivra 4 équipes tout le long du projet.

Chacun des 9 encadrants a proposé un sujet qui devra permettre l'application de l'ensemble des enseignements informatiques dispensés en 1ère année : 
...
Les sujets sont affectés aléatoirement aux équipes. La diversité des parcours (Anglais, Dauphine...), les délais restreints et les contraintes associées, font qu'il n'est pas possible de vous laisser faire des voeux.


### Première séance

De retour d'étudiants, la première rencontre entre membres d'une même équipe est importante. C'est l'occasion pour vous de faire connaissance et d'échanger honnêtement sur vos capacités et ce que vous attendez du projet. Les équipes solidaires qui ont un bon fonctionnement sont celles qui réussissent le mieux. Il n'y a pas de recette magique, chaque équipe doit se mettre d'accord sur son fonctionnement.
...
Chaque semaine (jusqu'au rendu du rapport final et sauf pendant les vacances scolaires), vous devrez remplir votre point hebdomadaire de suivi de projet :
...
- **Quoi** : listez les tâches accomplies + le temps global estimé
- **Pourquoi** : il permettra à votre tutrice ou tuteur de suivre l'évolution de votre travail

::: {.callout-important}
Le point hebdomadaire est **[obligatoire]{.underline}**. Vous serez pénalisés si vous ne le remplissez pas ou si vous le rendez en retard.
...
Dans l'équipe vous devez toutes et tous avoir une vision macro de l'avancement du projet et ce que vous avez prévu de faire.

**Une séance de suivi, ça se prépare !!!**

> C'est valable pour les autres projets, les suivi de stage, les réunions... Si vous arrivez les mains dans les poches, vous allez perdre un temps précieux pour vous remettre dans le bain.
...
**Les séances de suivi sont obligatoires.** L'appel sera effectué. Votre intervenant pourra demander à tout moment à parler avec la totalité des élèves qu'il encadre.


## Outils et Cours

### Module de Compléments d'informatique

Pour vous aider à mener à bien votre projet, un cours de 6h, et cinq TP de 3h sont mis en place dans le module de **Compléments d'informatique**. Lors du cours, vous seront présentés l'organisation générale du projet et quelques concepts nouveaux mis en œuvre lors des TP :
...
Sauf retour à l'âge de Pierre, vous allez inévitablement utiliser des applications informatiques dans votre vie professionnelle. Même si vous ne travaillez jamais côté informatique, avoir une première approche sur le fonctionnement d'un projet informatique vous sera précieux.
...
L'IA générative peut être un excellent accélérateur de développement lorsqu'elle est utilisée avec discernement. Si vous comprenez votre architecture, vos technologies et vos besoins, elle peut vous aider à produire du code plus rapidement, explorer des pistes ou résoudre des problèmes ponctuels.

Elle peut également vous aider à comprendre les principes évoqués en cours et en TP.

À l'inverse, lui demander de développer des fonctionnalités mal définies ou copier du code sans le comprendre risque de vous faire perdre beaucoup de temps. L'IA peut produire des solutions incorrectes, inadaptées à votre projet ou génératrices de bugs.

Considérez l'IA comme un assistant technique, pas comme un développeur autonome. Plus vos demandes sont précises et plus vous comprenez ce qu'elle propose, plus elle sera utile.
...
Cette 2e phase d'analyse aboutira à un modèle de conception pour répondre aux objectifs fixés par le cahier des charges. Le choix des diagrammes est fait par chaque équipe après validation de leur intervenant. Nous y trouverons cependant au minimum :
...
Tout au long du développement, vous développerez un ensemble de tests unitaires.
...
Via son rôle de client, votre tutrice ou tuteur vous a exposé son besoin. Le dossier d'analyse explique comment vous allez y répondre.
...
::: {.callout-important}
- Respectez les consignes usuelles de mise en forme d'un rapport
- Précisez votre numéro d'équipe sur la page de garde
:::
...
- La démarche d'assurance qualité. Le but de cette partie est de montrer que vous avez pris en compte les évènements extérieurs qui peuvent impacter votre application.
  ...
  - Si vous avez mis de côté des fonctionnalités au cours de votre développement, comment peuvent-elles être mise en place dans le futur ?
...
L'ordre de présentation des différents points, et de leur classement en annexe est libre. **La taille optimale de ce rapport est de 25 pages hors annexe**. Vous pouvez faire plus long mais dans ce cas posez-vous la question : « Ce que j'ajoute est-il nécessaire ? ». 

le format final attendu est le pdf.

Pour rédiger, il est vivement conseillé d'utilier les template Typst fourni dans le template de projet pour rédiger.

Enfin, un minimum de rigueur est attendu de futurs ingénieurs. Cela arrive de laisser passer une ou deux coquilles, par contre si cela nuit à la compréhension de votre rapport vous serez sanctionnés.
...
- Un fichier `README.md` à la racine du dépot [rédigé en anglais]{.underline} à destination d'un utilisateur et devra contenir :
  - une présentation rapide de votre application (principales fonctionnalités, besoins auquels elle répond)
  - un mode opératoire d'installation et de lancement rapide
- Un fichier `README.md` dans le dossier *backend* destiné aux personnes souhaitant continuer le développement de l'application :
  - Présentation de l'architecture en couches
  - Outils de développement (tests, logs, qualité...)
- Un fichier `backend/pyproject.toml` avec tous les modules pythons à installer pour faire fonctionner votre application
- un dossier `backend/src` contenant votre code source et un fichier `main.py` ou `__main__.py` qui permet de lancer votre application
...
::: {.callout-important}
Seul le code de la partie *backend* sera noté !

Si vous développez un *frontend*, vous pouvez donc le coder avec un peu moins de rigueur.
:::
...
- Vous pouvez utiliser des schémas, graphiques, captures d'écran utiles à la compréhension. Faites attention que vos images soient lisibles une fois projetées. 
...
Voir présentation aux élèves pour les dates

### Immersion

3 jours consécutifs sont libérés dans votre emploi du temps pour avancer efficacement sur votre projet. La présence dans l'école est obligatoire et sera contrôlée.
