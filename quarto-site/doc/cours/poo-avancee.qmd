...
::: {.notes}
Ce sont les trois principes qu'apporte la programmation orientée objet. Ils sont présentés par ordre de "simplicité". La qualité de votre code va dépendre de votre maîtrise et application de ces principes. Cela prend du temps (il m'a fallu environ 2 ans en tant que développeur pour bien les comprendre).
...
![](img/uml/class_diagram_no_poo.png)
...
:construction:
![](img/uml/class_diagram_poo1.png)
...
Une bonne pratique est d'associer à chaque concept concret un concept abstrait.  
Mais cela alourdit le code, alors on ne va pas faire ça systématiquement. On va faire des concepts abstraits qui ont du sens.
:::

### Par exemple

:construction:
![](img/uml/class_diagram_poo_abstract_class.png)
...
::: {.notes}
L'autre gros défaut de Python est son typage dynamique. Python va typer les objets au runtime, et pas au compile time. Cela ne permet pas d'avoir autant d'outils que dans d'autres langages. Mais vous pouvez utiliser les docstrings pour typer vos objets.
:::
...
:construction:
![](img/uml/class_diagram_poo_file_format.png)
...
:construction:
![](img/uml/class_diagram_poo_bridge_pattern.png)
...
![Génie logiciel](https://media.giphy.com/media/bU3bD2rsKq02Q/giphy.mp4){width=600}
...
Faire un dessin avec et sans. Expliquer que c'est le boulot d'un objet métier de dire comment il s'affiche et comment on le stocke. Pareil, ce n'est
:::
